...
The CWRU bearing dataset provides a standardized benchmark for comparison against published results. Smith and Randall \cite{smith2015RollingElementBearing} established this as the primary bearing fault benchmark. Rosa et al. \cite{rosa2024benchmarking} identified data leakage in typical evaluation protocols---same bearings appearing in train and test sets---that inflate reported accuracies from 69.5\% to 85.8\% for Naive Bayes classifiers.

\textbf{Test Configuration:}
\begin{itemize}
    \item Dataset: 1,189 samples (4 classes: normal, ball, inner race, outer race)
    \item Fault diameter: 0.007 inches, 12kHz sampling
    \item Split: 70\% train / 30\% test, stratified shuffle
    \item Buffer: 20 samples per labeling event
    \item Outlier threshold: 5.0$\times$ cluster radius
    \item Validation: 10 independent runs with different random seeds
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Comparison with Recent CWRU Baselines}
\begin{tabular}{lccc}
\toprule
Method & Accuracy & Classes & Memory \\
\midrule
Naive Bayes (Rosa 2024) & 69.5\% & 4-class & Batch \\
\textbf{TinyOL-HITL} & \textbf{75.8\% $\pm$ 6.8\%} & 4-class & O(1) \\
\bottomrule
\end{tabular}
\end{table}

Our streaming approach outperforms the Naive Bayes baseline by 6.3 percentage points while operating with O(1) memory per sample. Best run achieved 81.2\%, worst run 61.9\%.

\textbf{Confusion Matrix (Run 1, K=4):}

\begin{table}[h]
\centering
\begin{tabular}{l|cccc|c}
\toprule
 & Normal & Ball & Inner & Outer & Recall \\
\midrule
Normal & 125 & 1 & 0 & 0 & 99.2\% \\
Ball & 68 & 9 & 0 & 0 & 11.7\% \\
Inner & 0 & 11 & 73 & 0 & 86.9\% \\
Outer & 0 & 0 & 0 & 70 & 100\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Analysis:} Ball fault samples cluster with normal data---a known CWRU characteristic documented by Smith and Randall \cite{smith2015RollingElementBearing}. Ball defects produce subtle vibration signatures in time-domain features (RMS, kurtosis, crest factor) that overlap with normal operation. Frequency-domain analysis would improve separation but adds computational cost incompatible with streaming constraints.

The 75.8\% accuracy validates that label-driven incremental clustering achieves competitive performance against properly-evaluated baselines without pre-training, batch data requirements, or neural network complexity.
