...
\textbf{Principal Component Analysis (PCA)} \cite{sarita2022RETRACTEDPrincipalComponent} projects high-dimensional data to lower-dimensional space, detecting outliers via Mahalanobis distance. Effective for linear patterns but fails with non-linear machinery dynamics.
...
Industrial legacy systems use heterogeneous protocols developed over 40+ years. Zhang et al. \cite{zhang2020ArchitectureImplementationIndustrial} document IIoT gateway architectures bridging OT (Operational Technology) and IT (Information Technology) networks.

\textbf{OT protocols} \cite{zhang2020ArchitectureImplementationIndustrial}:
\begin{itemize}
...
\textbf{IT protocols} \cite{zhang2020ArchitectureImplementationIndustrial}:
\begin{itemize}
